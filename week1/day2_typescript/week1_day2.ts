...
const role: string = "Data Scientist";
const studyHours: number = 5;
const applicationsSent: number = 12;
const applications: number[] = [0, 1, 3, 7, 12];
const skills: string[] = ["python", "sql", "excel", "git", "aws"];


//demo code from the lesson
console.log(
  `Hi, my name is ${myname}. I'm aiming to become a ${role} and I study ${studyHours} hours per week.`
);
...
/* example of readonly string[]
interface User {
  readonly id: string; // This property cannot be changed after the object is created
  name: string; // This property can be changed
}
const user: User = {
  id: "user-abc-123",
  name: "John Doe",
...
// my readonly example for Use readonly string[]
interface AnotherInterface {
  readonly exampleReadOnly: string;
}

// Add a typed function
function myTypedFunction(name: string): string {
  return `Hello, ${name}!`;
}
